...
static int parse_document(yaml_document_t *doc, cf_sink_config_t *out, char *errbuf, size_t errcap)
{
     ...
     * derive it from the consumer group so each sink keeps its own dead-letter
     * stream (event -> cloudflow:v1:deadletter:sink-splunk). */
    v = scalar_val(map_get(doc, redis, "deadletter_stream"));
    if (v && *v) {
        out->redis.deadletter_stream = dup_str(v);
    } else if (out->redis.consumer_group) {
        size_t need = strlen(DEADLETTER_STREAM_PREFIX) + strlen(out->redis.consumer_group) + 1;
        out->redis.deadletter_stream = malloc(need);
        if (out->redis.deadletter_stream)
            snprintf(out->redis.deadletter_stream, need, "%s%s", DEADLETTER_STREAM_PREFIX,
                     out->redis.consumer_group);
    }
    if (!out->redis.deadletter_stream) {
    ...
    out->hec.hec_url = dup_str(v);
    /* Optional endpoint path appended to hec_url by the HEC client. Defaults
     * to "" so a hec_url that already carries the full path stays unchanged. */
    v = scalar_val(map_get(doc, splunk, "hec_path"));
    out->hec.hec_path = dup_str((v && *v) ? v : "");
    ...
    out->hec.hec_token_env = dup_str(v);

    v = scalar_val(map_get(doc, splunk, "index"));
    out->hec.index = dup_str((v && *v) ? v : "");

    out->hec.batch_size = scalar_int(splunk, doc, "batch_size", DEFAULT_BATCH_SIZE);
    out->hec.flush_interval_ms =
        scalar_int(splunk, doc, "flush_interval_ms", DEFAULT_FLUSH_INTERVAL_MS);
    out->hec.request_timeout_ms =
        scalar_int(splunk, doc, "request_timeout_ms", DEFAULT_REQUEST_TIMEOUT_MS);
    out->hec.tls_verify = scalar_is_truthy(scalar_val(map_get(doc, splunk, "tls_verify")), 1);

    if (!out->service.name || !out->service.consumer_name || !out->redis.consumer_group ||
        !out->hec.hec_url || !out->hec.hec_path || !out->hec.hec_token_env || !out->hec.index) {
        set_err(errbuf, errcap, "out of memory");
        return -1;
    }
...
static int load_from_parser(yaml_parser_t *parser, cf_sink_config_t *out, char *errbuf,
                            size_t errcap)
{
    yaml_document_t doc;
    int rc;
...
void cf_sink_config_free(cf_sink_config_t *cfg)
{
    ...
    free(cfg->redis.deadletter_stream);
    free(cfg->hec.hec_url);
    free(cfg->hec.hec_path);
    free(cfg->hec.hec_token_env);
    free(cfg->hec.index);
...
const char *cf_sink_resolve_token(const cf_hec_config_t *h)
{
    ...
    if (!h || !h->hec_token_env)
        return NULL;
    tok = getenv(h->hec_token_env);
    if (!tok || !*tok)
        return NULL;
    return tok;
}
