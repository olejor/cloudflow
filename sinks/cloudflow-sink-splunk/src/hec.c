                                   ...
                                   char *errbuf, size_t errcap)
{
    ...
    token = cf_splunk_resolve_token(splunk);
    if (!token) {
        if (errbuf && errcap)
            snprintf(errbuf, errcap, "HEC token env var %s is not set or empty",
                     splunk->hec_token_env ? splunk->hec_token_env : "(unset)");
        return NULL;
    }
