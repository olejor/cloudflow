...
fixture inputs for the metrics sink's golden tests (WP-M02,
docs/splunk-metrics.md).

Same pattern as the event sink's generator (sinks/cloudflow-sink-splunk/tests/
generate_fixtures.py): it is the *golden generator* -- the oracle that produces
the exact CloudFlowEvent bytes the committed tests/golden/*.jsonl goldens were
derived from, so the C metrics transform can be proven against the same inputs.

The protobuf Python bindings are shared with the event sink (they are the
committed cloudflow.v1 bindings under sinks/cloudflow-sink-splunk/src/
cloudflow_pb); this script locates them relative to the repo root so no
PYTHONPATH is needed in the checkout.
...
def _base_envelope(source_type, event_type, observed_ns, stream_name, source_host, payload_schema):
    return common_pb2.EventEnvelope(
        ...
        source_instance="metrics-itest-01",
...
def dns_transaction_service_role_event() -> CloudFlowEvent:
    ...
    service_role -- exercises the per-tier cf_service_role dimension plus
    cf_role/cf_rcode/cf_qtype/cf_server_ip and both DNS observed metrics
    (dns.transactions_total and dns.rtt_seconds)."""
...
def dhcpv4_discover_event() -> CloudFlowEvent:
    """A DHCPv4 DISCOVER (not relayed) -- exercises dhcp.events_total dimensioned
    by source_type / cf_message_type / cf_is_relayed."""
