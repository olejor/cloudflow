...
# tests/fixtures/dhcp/v4_*.pcap as a seed corpus, runs the harness over each
# seed, then generates 1000 deterministic truncation/bit-flip variants of
# those seeds and runs the harness over all of them. Prints a summary and
# exits non-zero if any invocation crashed.
#
# Usage: tests/fuzz/dhcpv4_fuzz_quick_check.sh
...
echo "== building ASan dhcpv4_fuzz =="
"${CC:-gcc}" -std=gnu11 -O1 -g -fsanitize=address,undefined -fno-omit-frame-pointer \
    ...
    -o "$WORK_DIR/dhcpv4_fuzz_asan" "$FUZZ_DIR/dhcpv4_fuzz.c" \
...
BIN="$WORK_DIR/dhcpv4_fuzz_asan"
SEEDS_DIR="$WORK_DIR/seeds"
mkdir -p "$SEEDS_DIR"

echo "== extracting seed payloads from tests/fixtures/dhcp/v4_*.pcap =="
...
count = 0
for path in sorted(glob.glob(os.path.join(fixtures_dir, "v4_*.pcap"))):
    pkts = rdpcap(path)
...
echo "== ASan-instrumented dhcpv4_fuzz quick check: PASS (no crash across $((seed_count + variant_count)) inputs) =="
